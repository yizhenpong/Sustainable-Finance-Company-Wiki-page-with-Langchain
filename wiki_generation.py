...
company_info_key_val_pairs = rag_chain.invoke(str(interested_fields))
...
There is also no start with "Title:", directly produce the contents.
...
for point in outline_ls:
    ...
    article = rag_chain2.invoke(f"What is generated for the pointer: {point}?")
...
#==================================================
'''retrieve & generate CONTENT for PART B 
- ESG Overview
    - commitment 
    - achievements
    - ESG ratings
...
'''prompt engineering'''
question3 = """Your task is to generate Environmental Social Governance (ESG) Overview of this company. 
            You may wish to cover their sustainable commitments like carbon zero or net zero,
            achievements, what are their current ESG reporting frameworks, their ESG ratings."""
...
You must not hallucinate and come up with your own content, 
a highly accurate answer that can be referenced to a page number is much preferred.
...
template4 = """Use the following pieces of context to answer the question at the end.
If you don't know the answer, just say that you don't know, don't try to make up an answer.
Come up with list of pointers for the question and keep the pointers as concise as possible
{context}
Question: {question}"""
...
TableOfContents = rag_chain.invoke("What is the table of contents? Return key value pairs where key = section_name, value = page_number")
dh.write_test_output(company_symbol,"Title: TableOfContents","TableOfContents")
dh.write_test_output(company_symbol,TableOfContents,"TableOfContents")

print("completed part 3 - idea 1a - generate table of contents")



'''idea 1b - generating table of contents, by common logic, it should be at the first few pages of the report'''
